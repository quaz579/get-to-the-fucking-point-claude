#!/usr/bin/env sh
# SessionStart hook: injects the full get-to-the-fucking-point-claude ruleset when the user has
# opted in by creating $CLAUDE_CONFIG_DIR/.get-to-the-fucking-point-claude-always (default ~/.claude).
...
flag_path="$claude_dir/.get-to-the-fucking-point-claude-always"
...
skill_path="$script_dir/../skills/get-to-the-fucking-point-claude/SKILL.md"
[ -f "$skill_path" ] || exit 0
...
printf 'BLUNT MODE ACTIVE (always-on). The ruleset below applies to every response. "stop adhd mode" turns it off for this session; delete %s to turn always-on off for good.\n\n%s\n' \
  "$flag_path" "$body"
